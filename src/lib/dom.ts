export class Container {
  ...
  constructor(public container: HTMLElement) {
    ...
    this.tileNodes = [
      (this.tiles[this.tiles.length - 1] as Tile).clone(),
      ...this.tiles,
      (this.tiles[0] as Tile).clone(),
    ];
    this.wrapTiles();
  }
  ...
  private get lastTileOffset() {
    return this.tiles[0] && this.tiles[0].offset;
  }
  ...
  getTileOffset(index: number) {
    ...
    for (let i = 0; i < index; i++) {
      offset += (this.tiles[i] as Tile).offset;
    }

    return -(this.lastTileOffset || 0) - offset;
  }

  get maxTopOffset() {
    return (
      -1 *
      (this.tiles.reduce((acc, { offset }) => acc + offset, 0) +
        (this.lastTileOffset || 0))
    );
  }
}
